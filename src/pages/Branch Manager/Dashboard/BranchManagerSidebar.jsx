...
import {
  ...
  Users, UserCircle, FileText, Settings, LogOut, X
} from "lucide-react";
...
const navLinks = [
  ...
  { name: "Refunds", path: "/branch/refunds", icon: <RefreshCw className="w-5 h-5" /> },
  { name: "Transactions", path: "/branch/transactions", icon: <CreditCard className="w-5 h-5" /> },
  { name: "Inventory", path: "/branch/inventory", icon: <Package className="w-5 h-5" /> },
  { name: "Employees", path: "/branch/employees", icon: <Users className="w-5 h-5" /> },
  { name: "Customers", path: "/branch/customers", icon: <UserCircle className="w-5 h-5" /> },
  { name: "Reports", path: "/branch/reports", icon: <FileText className="w-5 h-5" /> },
  { name: "Settings", path: "/branch/settings", icon: <Settings className="w-5 h-5" /> },
];

export default function BranchManagerSidebar({ isOpen, onClose }) {
      ...
      >
        ...
        <div className="mb-8 flex items-center justify-between">
          <div className="text-2xl font-extrabold text-primary tracking-tight flex items-center gap-2">
            ...
            <span className="text-lg">Branch Manager</span>
          </div>
          <Button
            variant="ghost"
            size="icon"
